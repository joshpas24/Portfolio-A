...
export default function VideoBackground({ background }) {
    ...
    return (
        <div className="absolute top-0 left-0 w-full h-full bg-black">
            <video className={`absolute top-0 object-cover h-full w-full ${isVisible ? 'opacity-100 transition-opacity duration-500' : 'opacity-0'}`} autoPlay controlsList="nodownload nofullscreen noremoteplayback" loop muted playsInline preload="auto" src={theme} type="video/mp4" />
        </div>
    )
}
